...
**Estado (2026-09-12):** Sprint 7 **fechada** (código + aceite local via `pnpm tester` — Pass=36 Fail=0). Cobrança SaaS permanece **manual** (ADR-0010; sem Stripe). M3 uso real S4 permanece pendente. Próxima: S8 (endurecimento / LGPD / piloto).
...
- [ ] Should stubs RF-E8-10 (deep-links metadata) se couber — **fora do DoD Must** (corte #15)
...
- [x] CSV `;` UTF-8 BOM; XLSX se lib leve já no monorepo — senão CSV Must + XLSX Should
...
- [x] Inbox: inbound webhook cria/atualiza conversa; send persiste OUT; outro tenant 404
- [x] Duplo POST message mesma Idempotency-Key = 1 mensagem
- [x] Dashboard 200 com KPIs inteiros (cents); dentista sem financial onde couber 403
- [x] Export 202 → READY → URL; CSV com `;` e BOM
...
- [x] Cross-tenant export/conversation → 404
...
- [x] Trial: banner com dias; após forçar expiry, clínica não cria paciente/agendamento; ainda lista e exporta
...
**Gate tester (2026-09-12):** `pnpm tester` Pass=36 Fail=0 Skip=1 (tunnel off). Smokes S7 (`messaging-inbox`, `reporting-dashboard`, `reporting-export`, `subscription`) + Playwright full verdes.
