...
import { pauseOutboxDispatch } from '../src/shared/database/outbox_dispatch_pause.js';
...
async function main() {
  const resumeOutbox = await pauseOutboxDispatch(300);
  let failed = false;
  ...
  try {
    ...
    await db.runProvisioning(async (tx) => {
      await tx.tenant.create({
        data: {
          ...
          // UUID v7 compartilha prefixo temporal — slug usa o id inteiro.
          slug: `outbox-${tenantId}`,
          updatedAt: new Date(),
        },
      });
    });
    ...
    // Isola deste tenant: backlog global + worker real (pausado) não devem entrar no batch.
    const tenantFilter = { tenantId };
    const dispatcher = new OutboxDispatcher(queue, db);
    const dispatched = await dispatcher.dispatchOnce(100, tenantFilter);
    await queue.drain();
    ...
    const whileDown = await dispatcher.dispatchOnce(100, tenantFilter);
    ...
    const afterUp = await dispatcher.dispatchOnce(100, tenantFilter);
    await queue.drain();
    ...
    const unknownDispatched = await dispatcher.dispatchOnce(100, tenantFilter);
